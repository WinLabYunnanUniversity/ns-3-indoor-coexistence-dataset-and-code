...
training_dir = "traindata.csv"        # 训练集地址
...
clf = RandomForestClassifier(n_estimators=65, max_depth=3, min_samples_leaf=6,
                             max_features=1, criterion="entropy"
                             )
# clf = DecisionTreeClassifier()
clf.fit(x_train, y_train)
y_pred = clf.predict(x_test)
score = clf.score(x_test, y_test)
scores = cross_val_score(clf, x, y, cv=5, scoring='accuracy').mean()
...
joblib.dump(clf, 'RadomForest.pkl')
#   restore
s = []
